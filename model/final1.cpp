...
float eye_x = 0, eye_y = 2500, eye_z = 3000;	//视觉中心位置
...
//float Shadow[pointLightNum-2][nearplane_height][nearplane_width][2];  //0对应模型，1对应面	
...
void cal_shadow(int lightNum, my_homogeneous_point p1, my_homogeneous_point p2, int model_num, int face_num)
{
	...
	for (int i = p1.x; i <= p2.x; i++)
	{
		...
		if (lightNum == 1) {
			if (x >= 0 && x < nearplane_width * 10 && y >= 0 && y<nearplane_height * 10 && z > Shadow_Depth1[y / 2][x / 2] && z < 0) {
...
void creatShadow(int lightNum) {
	...
	*/
	...
	shadowBuffer(lightNum, light[lightNum], Model);
	/*cout << "1" << endl;*/
}
//初始化三维图形顶点坐标
void init(void) {
	...
	if (num != 0) {			//天空盒分开来处理
		...
		for (int i = p1.x + 0.5; i <= p2.x + 0.5; i++)
		{
			...
			if (x < nearplane_width && x >= 0 && y < nearplane_height && y >= 0 && p1.z < 0) {
				/*int px = acp1[0].x+ 5*nearplane_width ;
				int py = acp1[0].y + 5*nearplane_height ;
				...
				for (int j = 0; j < pointLightNum; j++) {
					if (pointLight[j].isOpen == 1) {
						...
						if (j == 1) {
							...
							px = acp1[j - 1].x + nearplane_width / 2;
							py = acp1[j - 1].y + nearplane_height / 2;
						}
						/*if(Shadow[py][px][0]!=-1)
							cout << num << " " << face_num << " " << Shadow[py][px][0] << " " << Shadow[py][px][1] << endl;*/

						if (j == 0 || j == 1 && acp1[0].z < 0 && ((px >= 0 && px < 10 * nearplane_width && py >= 0 && py < 10 * nearplane_width) && (Shadow1[py / 2][px / 2][0] == num && Shadow1[py / 2][px / 2][1] == face_num/* || Shadow1[py][px][0] == -1 *//*Shadow_Depth1[py][px]<acp1[0].z+1*/))) {
							...
							//if (j == 1 && acp1[0].z < 0 && (px >= 0 && px < 10 * nearplane_width && py >= 0 && py < 10 * nearplane_width) && (/*Shadow1[py][px][0] != num && !Shadow1[py][px][1] != face_num */ Shadow_Depth1[py][px] > acp1[0].z -1)) {
							//	color[y][x][k].data[0] *=0.5 ;
							//	color[y][x][k].data[1] *= 0.5;
							//	color[y][x][k].data[2] *= 0.5;
							//}
						}
					}
				}
