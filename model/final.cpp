...
float eye_x =-544, eye_y = 2700, eye_z = 3195;	//视觉中心位置
float look_x =0.8089, look_y = 0, look_z =0.58;		//看向的方向
float pitch = 0, yaw = 2.5132;		//角度
int last_x = nearplane_width / 2, last_y = 0;

int speed = 300;		//视觉中心移动速度
float sensitivity = 0.002;		//鼠标输入灵敏度
int isperspective = 1;		//是否开启透视投影

int Shadow1[nearplane_height * 5][nearplane_width * 5][2];  //0对应模型，1对应面	
float Shadow_Depth1[nearplane_height * 5][nearplane_width * 5];//像素深度

float Shadow[pointLightNum - 2][2 * nearplane_height][2 * nearplane_width][2];  //0对应模型，1对应面	
float Shadow_Depth[pointLightNum - 2][2 * nearplane_height][2 * nearplane_width];//像素深度
...
vector<vector<my_face_homogeneous>> model_light[pointLightNum - 1];			//用来存灯视角下的坐标
...
//Color color[nearplane_height][nearplane_width][colorDepth];	//像素颜色 0是r 1是g 2是b
//float depth[nearplane_height][nearplane_width][colorDepth];//像素深度
//int Shadow[pointLightNum - 2][nearplane_height][nearplane_width][2];  //0对应模型，1对应面	
//float Shadow_Depth[pointLightNum - 2][nearplane_height][nearplane_width];//像素深度
...
void my_translate_homogeneous(vector<my_face_homogeneous >& model, float tx, float ty, float tz)
{
	//装配生成平移矩阵
	...
	for (int i = 0; i < model.size(); i++)
	{
		for (int j = 0; j < model[i].mList.size(); j++)
		{
			...
			input_v.ratio = 1;
			input_v = matrix_multiply_vector(translate_matrix, model[i].mList[j]);
...
void my_rotate_y_homogeneous(vector<my_face_homogeneous>& model, float angle)
{
	//装配生成旋转矩阵
	float translate_matrix[4][4];
	memset(translate_matrix, 0, sizeof(int) * 16);
...
void my_rotate(vector<my_face_homogeneous>& model, float angle)
{
	float sum_x = 0;
	float sum_y = 0;
	float sum_z = 0;
	...
	for (int i = 0; i < model.size(); i++)
	{
		for (int j = 0; j < model[i].mList.size(); j++)
		{
			sum_x += model[i].mList[j].x;
			sum_y += model[i].mList[j].y;
			sum_z += model[i].mList[j].z;
		}
	}

	float tx = -sum_x / model.size();
	float ty = -sum_y / model.size();
	float tz = -sum_z / model.size();
	//cout << tx << " " << ty << " " << tz << endl;
	my_translate_homogeneous(model, tx, ty, tz);
	my_rotate_y_homogeneous(model, angle);
	my_translate_homogeneous(model, -tx, -ty, -tz);
}
...
vector<vector<my_face_homogeneous>> translate_Model(vector<vector<my_face_homogeneous>>model, my_homogeneous_point light)
{
	for (int i = 0; i < model.size(); i++)
	{
		for (int k = 0; k < model[i].size(); k++) {
			for (int j = 0; j < model[i][k].mList.size(); j++)
			{
				model[i][k].mList[j].x -= light.x;
				model[i][k].mList[j].y -= light.y;
				model[i][k].mList[j].z -= light.z;
			}
			...
			{
				struct my_homogeneous_point input_v;
				input_v.x = model[i][k].mList[j].x;
				input_v.y = model[i][k].mList[j].y;
				input_v.z = model[i][k].mList[j].z;
				input_v.ratio = 1;

				input_v = matrix_multiply_vector(translate_matrix, input_v);
				model[i][k].mList[j].x = input_v.x;
				model[i][k].mList[j].y = input_v.y;
				model[i][k].mList[j].z = input_v.z;
				model[i][k].mList[j].ratio = input_v.ratio;
			}
			...
			for (int j = 0; j < model[i][k].mList.size(); j++)
			{
				...
				model[i][k].mList[j].z -= shadowDistance;
...
void cal_shadow(int lightNum, my_homogeneous_point p1, my_homogeneous_point p2, int model_num, int face_num)
{
	...
	for (int i = p1.x; i <= p2.x; i++)
	{
		...
		if (lightNum == 1) {
			x += 5 * nearplane_width;
			y += 5 * nearplane_height;
		}
		else if (lightNum > 1) {
			x += nearplane_width * 8;
			y += nearplane_height * 8;
		}
		if (lightNum == 1) {
			if (x >= 0 && x < nearplane_width * 10 && y >= 0 && y<nearplane_height * 10 && z > Shadow_Depth1[y / 2][x / 2] && z < 0 && mdelReflectivity[model_num].transparency == 1) {
				Shadow_Depth1[y / 2][x / 2] = z;
				//cout<< Shadow_Depth[y][x]<<" ";
				Shadow1[y / 2][x / 2][0] = model_num;
				Shadow1[y / 2][x / 2][1] = face_num;

				//cout << "shadow" << y << " " << x << endl;
			}
		}
		else if (lightNum > 1) {
			if (x >= 0 && x < 16 * nearplane_width && y >= 0 && y<16 * nearplane_height && z > Shadow_Depth1[y / 8][x / 8] && z < 0) {
				Shadow_Depth[lightNum - 2][y / 8][x / 8] = z;
				//cout<< Shadow_Depth[y][x]<<" ";
				Shadow[lightNum - 2][y / 8][x / 8][0] = model_num;
				Shadow[lightNum - 2][y / 8][x / 8][1] = face_num;
...
void scanline_filling_shadow(int lightNum, my_face_homogeneous p, int model_num, int face_num)
{
	...
	{                                                //找到所有两端点y坐标最小值为与当前扫描线ypos相同的所有边
		...
		for (int eindex = 0; eindex < AET.size(); eindex += 2)
		{
			...
			//放到容器里
			cal_shadow(lightNum, point1, point2, model_num, face_num);
			///
...
void ShadowCut(vector<my_face_homogeneous>& model, int mul) {
	for (int i = 0; i < model.size(); i++)
	{
		if (model[i].mList.size() == 3 && (model[i].mList[0].x > nearplane_width / 2 * mul && model[i].mList[1].x > nearplane_width / 2 * mul && model[i].mList[2].x > nearplane_width / 2 * mul ||
			model[i].mList[0].x <-nearplane_width / 2 * mul && model[i].mList[1].x <-nearplane_width / 2 * mul && model[i].mList[2].x <-nearplane_width / 2 * mul ||
			model[i].mList[0].x > nearplane_height / 2 * mul && model[i].mList[1].x > nearplane_height / 2 * mul && model[i].mList[2].x > nearplane_height / 2 * mul ||
			model[i].mList[0].x < -nearplane_height / 2 * mul && model[i].mList[1].x < -nearplane_height / 2 * mul && model[i].mList[2].x < -nearplane_height / 2 * mul ||
			model[i].mList[0].z >0 && model[i].mList[1].z >0 && model[i].mList[2].z >0))
			model[i].mList.clear();
	}
}
void shadowBuffer(int lightNum, my_homogeneous_point light, vector<vector<my_face_homogeneous>> model) {
	/*cout << light.x << " " << light.y << endl;*/
	//for (int i = 1; i < model.size(); i++)	//3：model数		//天空盒不用创造阴影
	//{
	//	for (int j = 0; j < model[i].size(); j++) {
	//		my_3Dvector v(model[i][j].center.x - light.x, model[i][j].center.y - light.y, model[i][j].center.z - light.z);
	//		if (model[i][j].n.dot_multiply(v) >= 0) {
	//			model[i][j].mList.clear();
	//		}
	//	}
	//}
	...
	for (int i = 1; i < model.size(); i++)	//3：model数		//天空盒不用创造阴影
	{
		...
		if (lightNum == 1)
			ShadowCut(model[i], 10);
		else if (lightNum > 1)
			ShadowCut(model[i], 16);
	}
	...
	for (int i = 1; i < model.size(); i++)
	{
		for (int j = 0; j < model[i].size(); j++)
		{
			if (model[i][j].mList.size() >= 3)
			{
				...
				scanline_filling_shadow(lightNum, model[i][j], i, j);
			}
		}
	}
}
void creatShadow(int lightNum) {
	...
	*/
	...
	if (lightNum == 1) {
		for (int u = 0; u < nearplane_width * 5; u++)
		{
			for (int v = 0; v < nearplane_height * 5; v++)
			{
				Shadow_Depth1[v][u] = -maxNum;
				Shadow1[v][u][0] = -1;
				Shadow1[v][u][1] = -1;
			}
		}
		light[1].x = pointLight[1].p.x;
		light[1].y = pointLight[1].p.y;
		light[1].z = pointLight[1].p.z;
		light[1].dx = -pointLight[1].p.x;
		light[1].dy = -pointLight[1].p.y;
		light[1].dz = 5000 - pointLight[1].p.z;
	}

	else if (lightNum > 1) {
		for (int u = 0; u < 2 * nearplane_width; u++)
		{
			for (int v = 0; v < 2 * nearplane_height; v++)
			{
				Shadow_Depth[lightNum - 2][v][u] = -maxNum;
				Shadow[lightNum - 2][v][u][0] = -1;
				Shadow[lightNum - 2][v][u][1] = -1;
			}
		}
	}
...
void init(void) {
	...
	for (int i = 1; i < 5; i++)
	{
		string url = "./海中屋/model/12_" + to_string(i);
		read(Model, url + ".obj");
		readMTL(mdelReflectivity, url + ".mtl");
		readNOR(url + ".mtl");
		cout << ModelNum << " " << url << endl;
	...
	for (int i = 1; i < 3; i++)
	{
		...
		read(Model, url + ".obj");
		readMTL(mdelReflectivity, url + ".mtl");
		mdelReflectivity[ModelNum - 1].transparency = 0.3;
		mdelReflectivity[ModelNum - 2].transparency = 0.3;
		mdelReflectivity[ModelNum - 3].transparency = 0.3;
		cout << ModelNum << " " << url << endl;
	...
	pointLight[0].p.x = 0;
	pointLight[0].p.y = 100000;
	pointLight[0].p.z = 0;
	/*pointLight[0].isOpen = 0;*/
	...
	while (getline(file, s))
	{
		...
		pointLight[index].p.x = atof(str[1].c_str()) * 20;
		pointLight[index].p.y = atof(str[2].c_str()) * 20;
		pointLight[index].p.z = atof(str[3].c_str()) * 20;
		/*pointLight[index].P.x = atof(str[1].c_str());
		pointLight[index].P.y = atof(str[2].c_str());
		pointLight[index].P.z = atof(str[3].c_str());*/
		pointLight[index].r = 1.25*1.4;
		pointLight[index].g = 0.901*1.4;
		pointLight[index].b = 0.776*1.4;

		pointLight[index].tag = 1;
		pointLight[index].Direction = my_3Dvector(0, -1, 0);
		pointLight[index].Cutoff = 25.0f / 360.0f * 3.14f;
		light[index].x = pointLight[index].p.x;
		light[index].y = pointLight[index].p.y;
		light[index].z = pointLight[index].p.z;
		light[index].dx = 0.05;
		light[index].dy = -0.99;
		light[index].dz = 0;
	}

	/*cout << light[4].x << " " << light[4].y << " " << light[4].z << " "<<endl;
	cout << light[9].x << " " << light[9].y << " " << light[9].z << " " << endl;
	cout << light[11].x << " " << light[11].y << " " << light[11].z << " " << endl;

	cout << pointLight[4].p.x << " " << pointLight[4].p.y << " " << pointLight[4].p.z << " " << endl;
	cout << pointLight[9].p.x << " " << pointLight[9].p.y << " " << pointLight[9].p.z << " " << endl;
	cout << pointLight[11].p.x << " " << pointLight[11].p.y << " " << pointLight[11].p.z << " " << endl;*/

	pointLight[9].Cutoff = 32;
...
void cal(my_homogeneous_point p1, my_homogeneous_point p2, my_homogeneous_point acp1[], my_homogeneous_point acp2[], int num, int face_num)
{
	...
	if (num != 0) {			//天空盒分开来处理
		float D_acpx[pointLightNum - 1], D_acpy[pointLightNum - 1], D_acpz[pointLightNum - 1];
		...
		for (int i = p1.x + 0.5; i <= p2.x + 0.5; i++)
		{
			...
			if (x < nearplane_width && x >= 0 && y < nearplane_height && y >= 0 && p1.z < 0) {
				...
				for (int j = colorDepth - 2; j >= k; j--) {
					...
					color[y][x][j + 1].data[3] = color[y][x][j].data[3];
				}
				dvector.dx = p1.dx;
				dvector.dy = p1.dy;
				dvector.dz = p1.dz;
				...
				for (int j = 0; j < pointLightNum; j++) {
					...
					if (pointLight[j].isOpen == 1&&(j==0||j==1||j==11||j==9||j==4)) {
						...
						if (j == 1) {
							...
							py = acp1[0].y + 5 * nearplane_height;
						}
						else if (j > 1) {
							px = acp1[j - 1].x + nearplane_width * 8;
							py = acp1[j - 1].y + nearplane_height * 8;
						}
...
void generate_ET(map<int, std::vector<Edge>>& ET, my_face_homogeneous polygon, int model_num, int face_num)
{
	...
	for (int eindex = 0; eindex < polygon.mList.size(); eindex++)
	{
		...
		top_point = startPoint.y > endPoint.y ? startPoint : endPoint;
		bottom_point = startPoint.y < endPoint.y ? startPoint : endPoint;
		if (model_num != 0) {			//天空盒不需要
			for (int i = 1; i < pointLightNum; i++) {
				my_homogeneous_point cur0, cur1;
				if (startPoint.y > endPoint.y) {
					...
					cur0 = model_light[i - 1][model_num][face_num].mList[eindex];
					cur1 = (eindex == (polygon.mList.size() - 1)) ? model_light[i - 1][model_num][face_num].mList[0] : model_light[i - 1][model_num][face_num].mList[eindex + 1];
				}
				else {
					...
					cur0 = (eindex == (polygon.mList.size() - 1)) ? model_light[i - 1][model_num][face_num].mList[0] : model_light[i - 1][model_num][face_num].mList[eindex + 1];
					cur1 = model_light[i - 1][model_num][face_num].mList[eindex];
				}
...
void scanline_filling(my_face_homogeneous p, int model_num, int face_num)
{
	...
	{                                                //找到所有两端点y坐标最小值为与当前扫描线ypos相同的所有边
		...
		{
			my_homogeneous_point point1, point2;
			my_homogeneous_point acpoint1[pointLightNum - 1], acpoint2[pointLightNum - 1];
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case'3':
	{
		for (int i = 2; i < 12; i++)
		{
			pointLight[i].isOpen = 1 - pointLight[i].isOpen;
		}
	break; 
	}
	case'4':
	...
	case 'G':
	{
		...
		cout << "eye:" << eye_x << " " << eye_y << " " << eye_z << endl;
		cout << pitch << " " << yaw << endl;
